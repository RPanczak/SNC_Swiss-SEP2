...
city_labels <- city_lines %>% 
  st_cast("POINT") %>% 
  group_by(name) %>%
  slice(seq(2, n(), by = 2)) %>% 
  ungroup()

city_points <- city_lines %>% 
  st_cast("POINT") %>% 
  group_by(name) %>%
  slice(seq(1, n(), by = 2)) %>% 
  ungroup()
...
median2_d <- 
  ...
  geom_sf_label(data = city_labels,
                label.size  = NA,
                aes(label = name),
                show.legend = FALSE,
                inherit.aes = FALSE) +
...
frq(sep3_grid_hex_500_sf$median_diff)
length(unique(sep3_grid_hex_500_sf$median_diff))
...
ggplot() + 
  ...
  theme(legend.position = c(.03, 1), 
        ...
        plot.title = element_text(hjust = 0.5, vjust = -5)) +
  guides(fill = guide_legend(override.aes = list(alpha = 1))) + 
  ggtitle("Swiss-SEP 3 - 2 difference")
